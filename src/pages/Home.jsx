...
export default function Home() {
  ...
  const [showInstructions, setShowInstructions] = useState(() => {
    return !localStorage.getItem('thrft_instructions_dismissed_home');
  });

  useEffect(() => {
    base44.auth.me().then(user => {
      if (user && !user.tour_completed) setShowTour(true);
    }).catch(() => {});
  }, []);
    ...
    <div>
      ...
      {lists.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-900 max-w-sm mb-6">Create your first grocery list to start comparing prices across stores.</p>
          <Link to="/NewList">
