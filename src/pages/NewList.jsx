...
export default function NewList() {
  ...
  const [showInstructions, setShowInstructions] = useState(() => {
    return !localStorage.getItem('thrft_instructions_dismissed_newlist');
  });
    ...
    <div>
      ...
      {isFree && !tierLoading && (
        <div className="mb-6 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-sm">
          ...
          <button onClick={() => setShowLimitModal(true)} className="text-xs font-semibold underline text-amber-700 hover:text-amber-900">
            Upgrade →
          </button>
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button type="button" onClick={() => setShowMethodPicker(!showMethodPicker)} className="flex items-center gap-1.5 text-xs font-medium text-blue-600 hover:text-blue-700 transition-colors">
        ...
        <div className="flex flex-col gap-3">
          ...
          />
          <PastListsDrawer onAddItems={(newItems) => setItems(prev => [...prev, ...newItems])} />
        </div>
        ...
        {items.length > 0 && (
          <Button onClick={handleCreate} disabled={saving} className="w-full h-14 rounded-xl text-base font-semibold shadow-lg shadow-blue-200 gap-2 transition-all" style={{ backgroundColor: '#4181ed' }}>
            {saving ? (
              <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              <>Save & Compare Prices <ArrowRight className="w-5 h-5" /></>
            )}
          </Button>
        )}
      </div>
